...
let filter = guestList.filter{ (guest) in
    return guest.age >= 18
}
...
let sortedGeust = guestList.sorted { $0.age >= $1.age }
print(sortedGeust)
...
let sumNew = randomNumbers.reduce(0) { $0 + $1 }
let sumNewOne = randomNumbers.reduce(0) { (result, item) in
    return result + item
}
...
print(numbersToFilter.filter { (number) in
    return number % 2 == 0
} )
...
print(numbersToDouble.map{ $0 * 2 })


//map the numbers into strings
let numbersToMapIntoStrings = [2, 4, 5, 1, 2, 2]
print(numbersToMapIntoStrings.map{ String($0) })
...
print(numbersToSum.filter{$0 > 0}.reduce(0, +))
