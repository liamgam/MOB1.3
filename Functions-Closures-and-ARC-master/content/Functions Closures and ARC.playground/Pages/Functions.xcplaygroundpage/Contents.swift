...
//write the function here
func printGuestName(guests: [Guest]) {
    for guest in guests {
        print(guest.name)
    }
}
...
printGuestName(guests: invitedGuests)
...
//write the function here
func returnOlderGuests(guest: [Guest]) -> [Guest] {
    return guest.filter{ $0.age >= 18 }
}
...
print(returnOlderGuests(guest: invitedGuests))
...
let numberCust = Customer.getNumberOfCustomers()
